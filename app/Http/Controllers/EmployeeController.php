...
class EmployeeController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('hr.employees.index')->with('success', __('Employee created successfully ,welcome email sent to employee'));
        } catch (\Exception $e) {
    ...
    public function liveLocationDashboard()
    {
        $employees = Employee::join('users', 'users.id', '=', 'employees.user_id')
            ->select('employees.id', 'users.name')
            ->get();

        return Inertia::render('hr/employees/LiveLocationDashboard', [
            'employees' => $employees
        ]);
    }
}
