...
public function exportAttendancePdf(Request $request)
{
    ...
    // Debug (agar phir bhi blank aaye to ye uncomment karke check karo)
    // dd($attendanceRecords->toArray());

    $html = view('exports.attendance-pdf', compact('attendanceRecords', 'year'))->render();

    return response($html)
        ->header('Content-Type', 'text/html')
        ->header('Content-Disposition', "attachment; filename=attendance-{$year}.html");
}
public function exportAttendanceCsv(Request $request)
{
    // Same base query as PDF
    $query = AttendanceRecord::with(['employee', 'shift', 'attendancePolicy', 'creator']);
    ...
    foreach ($records as $record) {
        $csvData[] = [
            ...
            // Date ko text force kar rahe hain (Excel #### issue fix)
            $record->date ? "\t" . \Carbon\Carbon::parse($record->date)->format('Y-m-d') : '',

            ucfirst($record->status),
            $record->clock_in ?? '',
            $record->clock_out ?? '',
            number_format($record->total_hours ?? 0, 2),
            number_format($record->overtime_hours ?? 0, 2),
            optional($record->shift)->name ?? '',
            optional($record->attendancePolicy)->name ?? '',
        ];
    }
