...
public function store(Request $request)
{
    ...
    $candidateFullName = trim($candidate->first_name . ' ' . $candidate->last_name);

    // ---------------------------------------------------
    // 1️⃣ Generate PDF and pass variables for Blade
    // ---------------------------------------------------
    ...
    $pdfContent = $pdf->output(); // PDF binary content

    // ---------------------------------------------------
    // 2️⃣ Send Email with PDF attachment
    // ---------------------------------------------------
    Mail::to($candidate->email)
        ->send(new OfferLetterMail($candidateFullName, $offer, $pdfContent));

    return redirect()->back()->with(
        'success',
        __('Offer created successfully & email sent to candidate')
    );
}
