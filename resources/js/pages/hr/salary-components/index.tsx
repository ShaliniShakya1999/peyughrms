...
export default function SalaryComponents() {
    ...
    const activeFilterCount = () => {
        return (
            (searchTerm ? 1 : 0) + (selectedType !== 'all' ? 1 : 0) + (selectedCalculationType !== 'all' ? 1 : 0) + (selectedStatus !== 'all' ? 1 : 0)
        );
    };
    ...
    const handleToggleStatus = (component: any) => {
        ...
        router.put(
            route('hr.salary-components.toggle-status', component.id),
            {},
            {
                onSuccess: (page) => {
                    toast.dismiss();
        ...
        },
        {
            ...
            render: (value: any, row: any) => (
                <span className="font-mono">
                    {row.calculation_type === 'fixed' ? window.appSettings?.formatCurrency(row.default_amount) : `${row.percentage_of_basic}%`}
                </span>
            ),
        },
        {
    ...
    return (
        <PageTemplate title={t('Salary Component Management')} url="/hr/salary-components" actions={pageActions} breadcrumbs={breadcrumbs} noPadding>
