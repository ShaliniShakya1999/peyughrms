...
export default function AttendanceRecords() {
    ...
    const handleExportPdf = () => {
        window.open(
            route('hr.attendance.export.pdf', {
  ...
  const handleExportCSV = () => {
    window.open(
        route('hr.attendance.export.csv', {
            employee_id: selectedEmployee || undefined,
            status: selectedStatus || undefined,
            date_from: dateFrom || undefined,
            date_to: dateTo || undefined,
    ...
    const activeFilterCount = () => {
        return (
            (searchTerm ? 1 : 0) + (selectedEmployee !== 'all' ? 1 : 0) + (selectedStatus !== 'all' ? 1 : 0) + (dateFrom ? 1 : 0) + (dateTo ? 1 : 0)
        );
    };
    ...
    const applyFilters = () => {
        router.get(
            route('hr.attendance-records.index'),
            {
                page: 1,
    ...
    const columns = [
        ...
        },
        {
            key: 'clock_in',
            label: t('Clock In'),
            render: (value: string) => <span className="font-mono text-green-600">{window.appSettings.formatTime(value) || '-'}</span>,
        },
        {
            key: 'clock_out',
            label: t('Clock Out'),
            render: (value: string) => <span className="font-mono text-red-600">{window.appSettings.formatTime(value) || '-'}</span>,
        },
        {
            key: 'total_hours',
            label: t('Total Hours'),
            render: (value: number) => <span className="font-mono">{Number(value).toFixed(2)}h</span>,
        },
        {
            key: 'overtime_hours',
            label: t('Overtime'),
            render: (value: number, row: any) => (
                <div className="text-sm">
                    <span className={`font-mono ${value > 0 ? 'text-orange-600' : 'text-gray-500'}`}>{Number(value).toFixed(2)}h</span>
                    {value > 0 && row.overtime_amount && (
                        <div className="text-xs text-green-600">{window.appSettings?.formatCurrency(row.overtime_amount)}</div>
                    )}
                </div>
            ),
        },
                ...
                return (
                    <div className="flex items-center gap-2">
                        ...
                        {value === 'on_leave' && row.leave_type && (
                            <span className="inline-flex items-center rounded-md bg-indigo-50 px-2 py-1 text-xs font-medium text-indigo-700 ring-1 ring-indigo-600/20 ring-inset">
                                {row.leave_type.name}
                            </span>
                        )}
                        {row.is_late && (
                            <span className="inline-flex items-center rounded-md bg-red-50 px-1 py-0.5 text-xs font-medium text-red-700 ring-1 ring-red-600/20 ring-inset">
                                {t('Late')}
                            </span>
                        )}
                        {row.is_early_departure && (
                            <span className="inline-flex items-center rounded-md bg-orange-50 px-1 py-0.5 text-xs font-medium text-orange-700 ring-1 ring-orange-600/20 ring-inset">
                                {t('Early')}
                            </span>
                        )}
                    </div>
